function XTourClient(requestURL, activeRequestLimit) {
	 ...
	 */
	this.planTours = function (planToursRequest , handler, timeout) {
		return sendRequest("planTours", planToursRequest, handler, timeout || this.planTours.timeout);
	};
	 ...
	 * @throws {object} only in the synchroneous mode: the error message from the server, or an Error object if a CORS request would be needed for an older IE browser.
	 */
	this.startPlanTours = function (planToursRequest , handler, timeout) {
		return sendRequest("startPlanTours", planToursRequest, handler, timeout || this.startPlanTours.timeout);
	};
	 ...
	 */
	this.runPlanTours = function (planToursRequest , handler, watchRequest, onUpdate, retryOptions) {
		return runJob(client.startPlanTours, planToursRequest, client.fetchToursResponse, handler, watchRequest, onUpdate, retryOptions);
	};	
	 ...
	 * @throws {object} only in the synchroneous mode: the error message from the server, or an Error object if a CORS request would be needed for an older IE browser.
	 */
	this.changeTours = function (changeToursRequest , handler, timeout) {
		return sendRequest("changeTours", changeToursRequest, handler, timeout || this.changeTours.timeout);
	};
	 ...
	 * @throws {object} only in the synchroneous mode: the error message from the server, or an Error object if a CORS request would be needed for an older IE browser.
	 */
	this.startChangeTours = function (changeToursRequest , handler, timeout) {
		return sendRequest("startChangeTours", changeToursRequest, handler, timeout || this.startChangeTours.timeout);
	};
	 ...
	 * @returns {object} the XMLHttpRequest object used, or null if the active request limit has been reached. 
	 */
	this.runChangeTours = function (changeToursRequest , handler, watchRequest, onUpdate, retryOptions) {
		return runJob(client.startChangeTours, changeToursRequest, client.fetchToursResponse, handler, watchRequest, onUpdate, retryOptions);
	};	
