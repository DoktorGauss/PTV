function XLoadClient(requestURL, activeRequestLimit) {
	 ...
	 */
	this.packBins = function (packBinsRequest , handler, timeout) {
		return sendRequest("packBins", packBinsRequest, handler, timeout || this.packBins.timeout);
	};
	 ...
	 * @throws {object} only in the synchroneous mode: the error message from the server, or an Error object if a CORS request would be needed for an older IE browser.
	 */
	this.startPackBins = function (packBinsRequest , handler, timeout) {
		return sendRequest("startPackBins", packBinsRequest, handler, timeout || this.startPackBins.timeout);
	};
	 ...
	 */
	this.runPackBins = function (packBinsRequest , handler, watchRequest, onUpdate, retryOptions) {
		return runJob(client.startPackBins, packBinsRequest, client.fetchPackedBinsResponse, handler, watchRequest, onUpdate, retryOptions);
	};	
