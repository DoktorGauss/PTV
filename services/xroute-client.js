function XRouteClient(requestURL, activeRequestLimit) {
    ...
    function findRequestURL(requestURL) {
        if (!requestURL || typeof requestURL !== "string") {
            return findBaseURL() + "rs/XRoute/";
        } else if(requestURL.startsWith("http")) {
            return requestURL;
        } else {
            return findBaseURL() + "rs/XRoute/" + requestURL + "/";
        }
    }
	 ...
	 */
	this.calculateRoute = function (routeRequest , handler, timeout) {
		return sendRequest("calculateRoute", routeRequest, handler, timeout || this.calculateRoute.timeout);
	};
	 ...
	 * @throws {object} only in the synchroneous mode: the error message from the server, or an Error object if a CORS request would be needed for an older IE browser.
	 */
	this.startCalculateRoute = function (routeRequest , handler, timeout) {
		return sendRequest("startCalculateRoute", routeRequest, handler, timeout || this.startCalculateRoute.timeout);
	};
	 ...
	 */
	this.runCalculateRoute = function (routeRequest , handler, watchRequest, onUpdate, retryOptions) {
		return runJob(client.startCalculateRoute, routeRequest, client.fetchRouteResponse, handler, watchRequest, onUpdate, retryOptions);
	};	
	 ...
	 * @throws {object} only in the synchroneous mode: the error message from the server, or an Error object if a CORS request would be needed for an older IE browser.
	 */
	this.fetchRouteResponse = function (id, onResult, timeout) {
		return sendRequest("fetchRouteResponse", { id: id }, onResult, timeout || client.fetchRouteResponse.timeout);
	};
